...
- If a standard command behaves unexpectedly, check whether it is aliased to a different tool with different flags. Either use the aliased tool's syntax or invoke the full path to bypass the alias.
...
- Do not leave debug statements or commented-out dead code in the final output unless asked.
...
- Apply DRY, KISS, SOLID principles where applicable. Do not over-engineer. When in doubt, ask for confirmation.
