...
#
# CLAUDE.md and KIRO.md must be byte-for-byte identical.
# AGENTS.md must match CLAUDE.md modulo its intentional Codex-only header
...
if [[ "$fail" -eq 0 ]]; then
  ...
  echo "Canonical source: agents/claude/CLAUDE.md"
  exit 1
fi
